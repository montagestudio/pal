...
var Slider = exports.Slider = Control.specialize({
            ...
            if (firstTime) {
                ...
                if(this.hasStandardElement) {
                    ...
                    while((iThumbElement = spacer.firstElementChild)) {
                        ...
                        iThumbWrapper.tabIndex = 0;
                        iThumbWrapper.onfocus = function (e) {


							var index = that._translateComposers.get(e.target.firstChild);
							//YUCK!!  ADDED THIS TO GRAB THE INDEX OUT OF THE ID (WHICH I HAD TO SET...) IN CASE WE CAN'T GET IT FROM TRANSLATECOMPOSERS
							if (index == undefined && e.target.id && (e.target.id.indexOf("-") != -1)) {
							    index = e.target.id.substring(e.target.id.indexOf("-")+1, e.target.id.length);
							}
							that._currentThumbIndex = index;
                    ...
                    while((iThumbWrapper = this.thumbWrappers[i])) {
                        ...
                        if (isHorizontal) {
                            //if (this._INSET_TRACKS) iThumbWrapper.style.marginLeft = (offset + (i ? iDimension/2.0 : 0)) + "px";
                            iThumbWrapper.style.marginLeft = (offset) + "px";
                        }
                        else {
                            //if (this._INSET_TRACKS) iThumbWrapper.style.marginTop = (offset + (i ? iDimension/2.0 : 0)) + "px";
            ...
            if(!this.hasStandardElement) {
                ...
                if (thumbWrappers && thumbWrappers.length > 0) {
                    ...
                    for(var i=0, iThumbElement, iTranslateComposer;(iThumbElement = thumbWrappers[i]);i++) {
                        ...
                        if (!iThumbElement.id || iThumbElement.id == "") iThumbElement.id = "thumb-"+i;
    ...
    _drawThumbElement: {
        value: function (thumbElementWrapper, thumbElement, index, isVertical, sliderMagnitude, length, cumulatedThumbSize, thumbElementOffsetSize) {
            var percent = this._percentageValues[index], position, positionString, trackElement = this.trackElements[index], thumbsize = 0;

            if(isVertical) {
                thumbsize = thumbElement.offsetHeight;
                ...
                trackElement.style.height = height+this._PERCENT_UNIT + (index > 0 ? thumbsize : thumbsize/2);
                ...
                if((index+1) === length) {
                    ...
                    trackElement.style.height = 100-percent+this._PERCENT_UNIT + (thumbsize/2);
                }

            }
            else {
                thumbsize = thumbElement.offsetWidth;
                console.log(index + " thumbsize = " + thumbsize + " cum=" + cumulatedThumbSize + " elem = " + thumbElementOffsetSize);
                ...
                if (this._INSET_TRACKS) {
					trackElement.style.left = "calc(" + (trackElementLeftPercent+this._PERCENT_UNIT) + " - " + (index > 0 ? thumbsize/2 : 0) + "px)";
					trackElement.style.marginLeft = cumulatedThumbSize+this._PIXEL_UNIT;
					trackElement.style.width = "calc(" + (width+this._PERCENT_UNIT)  + " + " + (index > 0 ? thumbsize : thumbsize/2) + "px)";
                } else {
                ...
                if((index+1) === length) {
                    ...
                    //We need the size of user-land element.
                    if (this._INSET_TRACKS) {
						trackElement.style.left = "calc(" + (percent+this._PERCENT_UNIT) + " - " + (thumbsize/2) + "px)";
						trackElement.style.marginLeft = (cumulatedThumbSize + thumbElementOffsetSize)+this._PIXEL_UNIT;
						trackElement.style.width = "calc(" + (100-percent+this._PERCENT_UNIT) + " + " + (thumbsize/2) + "px)";
                    } else {
        ...
        value: function () {
            if(!this.hasStandardElement) {
                ...
                for(var i=0, iThumbElementWrapper, iThumbElement, iThumbElementOffsetSize, countI = this.thumbWrappers.length, cumulatedThumbSize = 0;(iThumbElementWrapper = this.thumbWrappers[i]);i++) {
                    ...
                    this.thumbElements[i].classList.remove("left");
                    this.thumbWrappers[i].classList.remove("left");
                    this.thumbElements[i].classList.remove("right");
                    this.thumbWrappers[i].classList.remove("right");
                    if (i && (this._percentageValues[i] == this._percentageValues[i-1])) {
                        this.thumbElements[i-1].classList.add("left");
                        this.thumbElements[i].classList.add("right");
                        this.thumbWrappers[i-1].classList.add("left");
                        this.thumbWrappers[i].classList.add("right");
                    }
                    this.thumbWrappers[i].style.marginLeft = (offset) + "px";

                    offset += iThumbElementOffsetSize;
                }
                //offset = 0;
                if (!isVertical) this._spacer.style.marginRight = (this._spacerMarginEnd = offset) + "px";
                else this._spacer.style.marginTop = (this._spacerMarginEnd = offset) + "px";
    ...
    handleTranslateStart: {
        value: function (e) {
            this.rootComponent.classList.add("selectnone");
            this.active = true;
    ...
    handleTranslateEnd: {
        value: function (e) {
            this.rootComponent.classList.remove("selectnone");
    ...
    handleKeyPress: {
        value: function (event) {
            //alert(this.enabled + " " + event.target.identifier);
            if (!this.enabled) {
                return;
            }
            if(event.target.identifier === "increase") {
                this._increase();
            } else if (event.target.identifier === "decrease") {
                this._decrease();
            }
...
*/
    ...
    handlePropertyChange: {
        value: function (changeValue, key, object) {
            console.log(changeValue + " " + key + " " + object);
